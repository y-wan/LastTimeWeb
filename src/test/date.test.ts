...
import { averageInterval, elapsedParts, formatElapsed, formatInterval, formatSyncDateTime, formatSyncTime, historyGroup } from '../date'
...
describe('elapsed date behavior', () => {
  ...
  it('uses the Android never-recorded wording for event values', () => {
    expect(formatElapsed(undefined, 'en')).toBe('Never recorded')
    expect(formatElapsed(undefined, 'zh-CN')).toBe('尚未记录')
  })
